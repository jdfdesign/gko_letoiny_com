jQuery(document).ready(function() { 
  ...
  .on("ajax:beforeSend", function(evt, xhr, settings) {
    ...
    var first_name = jQuery('#inquiry_first_name').val();
    if (first_name == "" || first_name == " ") {
      jQuery('#inquiry_first_name').after("<span class='error'></span>");
      jQuery('#inquiry_first_name').parent().find('.error').fadeIn('slow');
      error = true;
    } else {
      jQuery('#inquiry_first_name').after("<span class='valid'></span>");
      jQuery('#inquiry_first_name').parent().find('.valid').fadeIn('slow');
    }
    
    var last_name = jQuery('#inquiry_last_name').val();
    if (last_name == "" || last_name == " ") {
      jQuery('#inquiry_last_name').after("<span class='error'></span>");
      jQuery('#inquiry_last_name').parent().find('.error').fadeIn('slow');
      error = true;
    } else {
      jQuery('#inquiry_last_name').after("<span class='valid'></span>");
      jQuery('#inquiry_last_name').parent().find('.valid').fadeIn('slow');
    }
